...
const socialLinkSchema = z.object({
  ...
  platform: z.enum(["instagram", "tiktok", "facebook", "x", "youtube", "linkedin", "whatsapp"]),
...
export async function createAdminSocialLinkAction(input: AdminSocialLinkInput) {
  ...
  const link = await prisma.socialLink.create({
    data: { platform: data.platform, url: data.url, icon: data.icon },
  });
...
export async function updateAdminSocialLinkAction(input: AdminSocialLinkInput) {
  ...
  const link = await prisma.socialLink.update({
    where: { id },
    data: { platform: normalized.platform, url: normalized.url, icon: normalized.icon },
  });
