...
export function CustomerSignUpForm() {
  ...
  const handleGoogleSignUp = () => {
    startGoogleTransition(async () => {
      await signUpWithGoogleAction(redirectUrl);
    });
  };
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      {/* Google Sign-Up Button - Large & Prominent */}
      <div>
        ...
        >
          ...
          {googlePending ? "Creating account..." : "Sign up with Google"}
        </button>
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center text-sm">
          <span className="px-2 bg-[#05060a] text-white/45">Or sign up with email</span>
        </div>
      </div>
      ...
      >
        {isPending && <RippleSpinner size={28} color="currentColor" label="Creating account" />}
      ...
      <div className="text-center text-sm text-white/60">
        ...
        <Link
          href={`/sign-in?callbackUrl=${encodeURIComponent(redirectUrl)}`}
